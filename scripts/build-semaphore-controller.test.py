#!/usr/bin/env python3
"""Exercise build-semaphore-controller: pure policy first, machinery second.

    python3 scripts/build-semaphore-controller.test.py

TWO LAYERS, tested two different ways.

The POLICY -- every TIGHTEN/LOOSEN/HOLD/RECLAIM decision, the high-water mark,
the burst and absorbable clamps, the resident arithmetic -- lives in decide(),
a pure function of (State, Inputs, cfg) with the clock injected as `now`.
Those branches are asserted below as instant unit tests: no subprocess, no
sleeping through dwell or grant intervals, and therefore immune to how loaded
the box happens to be. This mirrors kx-build-slot.test.py, which writes the
state file by hand for the same reason: behaviour that can be tested without
the clock should be.

The MACHINERY -- flock reconciliation, the published state file, resident
marker pruning, SIGTERM restoring full capacity -- genuinely involves the
kernel and a second process, so the integration phase at the end runs the real
controller against a synthetic pool. It polls for convergence with generous
timeouts rather than asserting on dwell-scale timing, so a busy machine slows
it down without failing it.
...
# ---------------------------------------------------------------------------
# UNIT TESTS: decide() with hand-written inputs and an injected clock.
...
# ---------------------------------------------------------------------------
# INTEGRATION: the real subprocess against a synthetic pool. Three phases:
# (a) startup -> publish, (b) flock reconcile against busy slots, (c) resident
# marker pruning, then SIGTERM restoring full capacity. Everything is
# convergence-polled; nothing asserts on dwell or grant-scale timing (ramp
# shape is decide()'s business, unit-tested above).
...
ENV.update(
    ...
    # in one step: ramp pacing is unit-tested against decide() above, so the
    # subprocess run only has to exercise the machinery around it.
...
def free_paths():
    ...
    impolite here, it CORRUPTS THE RUN: a test-and-release shows up as a rise
    in occupancy, and the controller reads a rise as an admission and restarts
    the spacing clock. The same argument is why the controller and the status
    bar both read this file instead.
    """
    locked = locked_keys()
    out = []
...
def wait_for(cond, timeout=20.0):
    """Poll until cond() is truthy. Generous timeout, tolerant of load."""
...
try:
    ...
    # hostile the pool (the occupied - resident guard, unit-tested above).
    check("returned slots are reclaimed down to the floor",
          wait_for(lambda: state()[1:2] == ["1"]
    ...
    check("live keeper counted and published",
          wait_for(lambda: state()[6:7] == ["1"]), True)
    ...
    check("stale marker pruned from disk",
          wait_for(lambda: not (SEM / "resident" / "slot.01").exists()), True)
    check("stale marker not counted",
          wait_for(lambda: state()[6:7] == ["0"]), True)
    ...
    check("every slot free after stop",
          wait_for(lambda: len(free_paths()) == 16, timeout=5), True)
