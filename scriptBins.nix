...
(pkgs.writeScriptBin "rush-pnpm" ''
    ...
    (pkgs.writeScriptBin "claude-agents" ''
      ...
      exec "$claude" agents "$@"
    '')
    (pkgs.writeScriptBin "claude-usage" ''
      #!/usr/bin/env bash
      ...
      cache_age() {
        if [ -f "$CACHE" ]; then
          local m; m=$($cu/stat -c %Y "$CACHE" 2>/dev/null || echo 0)
          echo $(( $($cu/date +%s) - m ))
        else
          echo 999999
        fi
      ...
      fmt_reset() {
        ...
        if printf '%s' "$raw" | $grep -qE '^[0-9]+$'; then
          ...
          epoch=$($cu/date -d "$raw" +%s 2>/dev/null) || epoch=""
        fi
        ...
        when=$($cu/date -d @"$epoch" '+%Y-%m-%d %H:%M' 2>/dev/null || echo "$raw")
